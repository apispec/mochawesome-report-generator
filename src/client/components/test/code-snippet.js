...
import './highlight.js.global.css';
...
const CodeSnippet = React.memo(props => {
  const { code, highlight, lang } = props;
  const shouldHighlight = !(lang === 'diff' && isArray(code)) && (code && highlight);
  ...
  const preRef = useCallback(node => {
    if (node && shouldHighlight) {
      hljs.highlightBlock(node);
    }
  }, [shouldHighlight]);

  const { label, showLabel, styles } = props;
  ...
  const renderLegendRight = () =>
    isInlineDiff ? (
      <DiffExpected inline={isInlineDiff}>{'expected\n\n'}</DiffExpected>
    ) : (
  ...
  return (
    !!code && (
      <Pre ref={preRef} styles={styles} diff={isDiff} className={classNames}>
        <code>
